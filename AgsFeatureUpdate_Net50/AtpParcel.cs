...
namespace geographia.ags
{
    ...
    {
        const string _PARCEL_KEY = "Assessor_Parcel_Number";
        ...
        public async Task<IEnumerable<Status_dto>> GetParcels(string pid)
        {
            ...
            var req = $"{_URL}/{_LAYERID}/query?f=json&where=PARCEL_ID%3D'{pid}'&returnGeometry=false&returnIdsOnly=false&outFields=OBJECTID,{_PARCEL_KEY},ParcelStatus,ROE_Status,Documents";
            //var r = await GetAll<Status_dto>(req, (arr) =>
            //{
            //    var list = new List<Status_dto>();
        ...
        public async Task<IEnumerable<DomainValue>> GetDomainValues(int layerId)
        {
            ...
            var m = JsonSerializer.Deserialize<MapD>(desc);
