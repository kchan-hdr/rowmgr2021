...
namespace ROWM.Dal
{
    public class StatisticsRepository : IStatisticsRepository
    {
        ...
        // hard coded. this is not likely to be used
        ...
        protected IQueryable<Parcel> ActiveParcels() => _context.Parcels.Where(px => px.IsActive);
        ...
        public async Task<(int nParcels, int nOwners)> Snapshot(int? part = null)
        {
            ...
            var owners = actives.SelectMany(px => px.Ownerships.Select(ox => ox.OwnerId));
        ...
        {
            var q = await (from p in ActiveParcels(part)
                           group p by p.LandownerScore ?? 0 into psg
        ...
        public async Task<Financials> GetFinancials(int? part = null)
        {
            ...
            {
                Appraisal = p.Sum(px => px.InitialRoeofferOfferAmount) ?? 0,
                Settlement = p.Sum(px => px.FinalRoeofferOfferAmount) ?? 0,
                ApprovedOffer = p.Sum(px => px.InitialOptionOfferOfferAmount) ?? 0,
                Sales = p.Sum(px => px.FinalOptionOfferOfferAmount) ?? 0,
                Closing = p.Sum(px => px.InitialEasementOfferOfferAmount) ?? 0
            };
        }

        #region helper
        private IEnumerable<SubTotal> MakeBaseParcels() => _context.ParcelStatuses.Where(px => px.IsActive && px.Category == "acquisition").OrderBy(px => px.DisplayOrder).Select(px => new SubTotal { Title = px.Code, Caption = px.Description, DomainValue = px.DomainValue.ToString(), Count = 0 }).ToArray();
        private IEnumerable<SubTotal> MakeBaseRoes() => _context.ParcelStatuses.Where(px => px.IsActive && px.Category == "roe").OrderBy(px => px.DisplayOrder).Select(px => new SubTotal { Title = px.Code , Caption = px.Description, DomainValue = px.DomainValue.ToString(), Count = 0 }).ToArray();
        private IEnumerable<SubTotal> MakeBaseClearances() => _context.ParcelStatuses.Where(px => px.IsActive && px.Category == "clearance").OrderBy(px => px.DisplayOrder).Select(px => new SubTotal { Title = px.Code, Caption = px.Description, DomainValue = px.DomainValue.ToString(), Count = 0 }).ToArray();

        private async Task<IEnumerable<SubTotal>> MakeBaseCounts(string cat) =>
            await _context.ParcelStatuses.Where(px => px.IsActive && px.Category == cat)
